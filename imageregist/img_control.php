 ...
 */
	 ...
	 * @param $img_file 読み込み対象の画像ファイルのパス
	 * @param $img_type Optional. 読み込み対象の画像ファイルの拡張子。引数を省略した場合画像の拡張子は自動的に判定される。引数を省略しなかった場合、判定処理は省略される。
...
//				break;
	...
	function oneDimensionRGBforRLE($img,$img_length,$arr_header)
	{
		...
		$selectionIndex=0;
		$beforeColor=imagecolorat($img,getXfromArrayHeader($arr_header,0,$width),getYfromArrayHeader($arr_header,$selectionIndex,$width,$height));
