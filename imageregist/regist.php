 ...
 */
	 ...
	 * @param string $input_file_imgtn POSTするFormData型変数のappend関数に追加するfile型の画像データのFormData上での名前。
	 * @param string $input_text_usertn POSTするFormData型変数のappend関数に追加するstring型のusername変数のFormData上での名前。
	 * @param string $save_dir 画像アップロード認証を使用する際、そのためのデータを格納するためのディレクトリのパスを表すstring型の変数。
	 */
	function regist($input_file_imgtn,$input_text_usertn) {
		if(regist_check($input_file_imgtn,$input_text_usertn)){
			print(regist_save($_FILES[$input_file_imgtn]['tmp_name'],$_POST[$input_text_usertn]));
		} else {
			print("ID登録に失敗しました。");
		};
	}
...
//ちゃんと中身を入れたか確認
			...
			$ucheck=username_check($user_name);
			if($ucheck) {
	 ...
	 */
	function regist_save($img_file,$user_name) {
	 ...
	 * @param string $img_file 認証用画像ファイルのパス
	 * @param string $key_hash_path 認証用画像ファイルのパス
